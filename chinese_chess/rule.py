    ...
    if GUI.game.player:
        ...
        if GUI.game.first:
            if GUI.game.mode in 'LAN COMPUTER END':
                statistic(First=1)
            GUI.game.player = '我方' if GUI.game.mode == 'LAN' else '玩家' if GUI.game.mode in 'COMPUTER END' else '红方'
        else:
            GUI.game.player = '对方' if GUI.game.mode == 'LAN' else '电脑' if GUI.game.mode in 'COMPUTER END' else '黑方'
...
def revoke(flag: bool = False) -> None:
    ...
    if flag or (GUI.game.player and GUI.game.mode in 'LOCAL' and GUI.game.index >= 0):
        if GUI.game.choose:
            ...
            GUI.game.choose = None
        o, pos, v, = GUI.game.cache[GUI.game.index]
        ...
        switch()
        statistic(Revoke=1)
    elif GUI.game.mode in 'COMPUTER END' and GUI.game.player == '玩家' and GUI.game.index >= 0:
        revoke(True)
...
def recovery(flag: bool = False) -> None:
    """ 恢复（悔棋） """
    import GUI
    if flag or (GUI.game.player and GUI.game.mode == 'LOCAL' and -1 <= GUI.game.index < len(GUI.game.cache)-1):
        if GUI.game.choose:
            GUI.game.choose.virtual_delete()
            GUI.game.choose.highlight(False, inside=False)
            GUI.game.choose = None
        GUI.game.index += 1
        o, pos, v, = GUI.game.cache[GUI.game.index]
        GUI.game.chesses[pos[1] + v[1]][pos[0] + v[0]].move(
            bool(o), -v[0], -v[1], True)
        GUI.game.chesses[pos[1] + v[1]][pos[0] + v[0]] = None
        switch()
        statistic(Recovery=1)
    elif GUI.game.mode in 'COMPUTER END' and GUI.game.player == '玩家' and -1 <= GUI.game.index < len(GUI.game.cache)-1:
        recovery(True)
