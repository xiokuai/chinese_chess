...
class Tk(tkinter.Tk):
    ...
    def wm_geometry(self: Self, newGeometry: str | None = None) -> str | None:
        ...
        if newGeometry:
            width, height, _width, _height, *_ = map(
                int, (newGeometry + "+0+0").replace("+", "x").split("x")
            )
        ...
        width, height, _width, _height, *_ = map(
            int, (geometry + "+0+0").replace("+", "x").split("x")
        )
...
class Canvas(tkinter.Canvas):
    ...
    def __init__(
        ...
        **kw,
        ...
        self.rate_x = 1.0  # 横向放缩比率
        self.rate_y = 1.0  # 纵向放缩比率
    ...
    def zoom(
        self: Self, rate_x: float | None = None, rate_y: float | None = None
    ) -> None:
        """
        缩放画布及其内部的所有元素
        ...
        for item in self.find_all():  # item 位置缩放
            self.coords(
                item,
                *[c * (rate_x, rate_y)[i & 1] for i, c in enumerate(self.coords(item))],
            )
    ...
    def moveto(
        ...
        # 重写：兼容不同的DPI缩放
        return tkinter.Canvas.moveto(
            self, tagOrId, x * S if x else x, y * S if y else y
        )

    def bbox(
        self: Self, *args  # type: str | tkinter._CanvasItemId
    ...
    def __init__(
        ...
        if radius:
            ...
            self.inside = [  # 虚拟控件内部填充颜色
                canvas.create_rectangle(x, _y, x + width, y + height - radius, **kw),
                canvas.create_rectangle(_x, y, x + width - radius, y + height, **kw),
                canvas.create_arc(x, y, x + d, y + d, start=90, **kw),
                canvas.create_arc(x + _w, y, x + width, y + d, start=0, **kw),
                canvas.create_arc(x, y + _h, x + d, y + height, start=180, **kw),
                canvas.create_arc(
        ...
        self.text = canvas.create_text(  # 虚拟控件显示的文字
            ...
            + (
                radius + 2
                if justify == "left"
                else width - radius - 3 if justify == "right" else width / 2
            ),
            y + height / 2,
            text=text,
            font=font,
            justify=justify,
            anchor=(
                "w" if justify == "left" else "e" if justify == "right" else "center"
            ),
            fill=color_text[0],
        )

    def state(
        self: Self, mode: Literal["normal", "touch", "press", "disabled"] | None = None
    ) -> None:
        """
        ...
        if self.radius:
            ...
            for item in self.outside[4:]:
                self.master.itemconfigure(item, outline=self.color_outline[mode])
        else:
...
class CanvasLabel(_BaseWidget):
    ...
    def touch(self: Self, event: tkinter.Event) -> bool:
        """触碰状态检测"""
        condition = (
            self.x1 <= event.x / S <= self.x2 and self.y1 <= event.y / S <= self.y2
        )
...
class _TextWidget(_BaseWidget):
    ...
    def cursor_update(self: Self, text: str = " ") -> None:
        ...
        if isinstance(self, CanvasEntry):
            self.master.coords(
                self._cursor,
                self.master.bbox(self.text)[2],
                self.y1 + self.height * self.master.rate_y / 2,
            )
...
class CanvasEntry(_TextWidget):
    ...
    def input(self: Self, event: tkinter.Event) -> None:
        ...
        if self._state == "press":
            ...
            self._value[0] = (
                len(self.value) * self.show  # 更新表面显示值
                if self.show
                else self.value
            )
...
class CanvasText(_TextWidget):
    ...
    def __init__(
        ...
        _x = x + (
            width - radius - 3
            if justify == "right"
            else width / 2 if justify == "center" else radius + 2
        )
    ...
    def input(self: Self, event: tkinter.Event) -> bool:
        ...
        if self._state == "press":
            if event.keysym == "BackSpace":  # 按下退格键
                ...
                if (
                    _pos[2] > self.x2 - self.radius - 2
                    or _pos[0] < self.x1 + self.radius + 1
                ):  # 文本溢出啦
    ...
    def input_backspace(self: Self) -> None:
        ...
        if _text:  # 正常地删除字符
            ...
            if self.value == self.master.itemcget(self.text, "text"):
                ...
                text = self.master.itemcget(self.text, "text")
                __ = (
                    self.value.removesuffix(text)[:-1].rsplit("\n", 1)[-1]
                    + "\n"
...
class PhotoImage(tkinter.PhotoImage):
    ...
    def __init__(self: Self, file: str | bytes, **kw) -> None:
        """
        `file`: 图片文件的路径
    ...
    def parse(self: Self, start: int = 0) -> Generator[int, None, None]:
        ...
        try:
            while True:
                self.image.append(
                    tkinter.PhotoImage(file=self.file, format="gif -index %d" % start)
                )
    ...
    def play(
        ...
        **kw,
        ...
        canvas.after(
            interval,
            lambda: self.play(  # 迭代执行函数
                canvas,
                item,
                interval,
                _ind=0 if (_ind := kw["_ind"] + 1) == len(self.image) else _ind,
            ),
        )
    ...
    def zoom(
        self: Self, rate_x: float, rate_y: float, precision: float | None = None
    ) -> tkinter.PhotoImage:
        """
        缩放图片
        `rate_x`: 横向缩放倍率
        ...
        if precision != None:
            ...
            for x in range(width):
                for y in range(height):
                    image.put(
                        "#%02X%02X%02X" % self.get(int(x / rate_x), int(y / rate_y)),
                        (x, y),
                    )
...
def move(
    ...
    if _ind:  # 记忆值
        ...
        return move(
            master, widget, dx, dy, times, (math.cos, 0, 0.6 * math.pi), frames, end
        )
    else:  # 函数模式
    ...
    elif isinstance(master, tkinter.Misc) and isinstance(
...
def text(
    length: int, string: str, position: Literal["left", "center", "right"] = "center"
) -> str:
    """
    ### 文本函数
...
def color(color: Iterable[str] | str, proportion: float = 1.0) -> str:
    """
    ### 颜色函数
